...
protected:
	int tree_depth(const PtrToNode ptn) const;
...
template<typename T>
void Binary_tree<T>::delete_tree(Binary_tree<T>::PtrToNode ptn)
{
	if (ptn == nullptr)
		delete ptn;
...
template<typename T>
int Binary_tree<T>::tree_depth(const Binary_tree<T>::PtrToNode ptn) const
{
	int ldepth, rdepth;
	if(ptn == nullptr)
		return 0;
...
void Binary_tree<T>::print_tree(std::ostream& output, 
							    Binary_tree<T>::PtrToNode ptn, int depth) const
{
	if (ptn == nullptr)
		return;
